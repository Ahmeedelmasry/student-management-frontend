    ...
    <v-row class="mb-4 align-center flex-grow-0">
      <v-col cols="6">
        <h2 class="text-h4 font-weight-bold text-auto">
          <v-icon class="text-auto me-2">mdi-account-cog</v-icon>
          ادارة المديرين
        </h2>
      </v-col>
    ...
    <v-card class="flex-grow-1">
      ...
      >
        <template #item.isAdmin="{ item }">
          <v-chip
            :color="item.isAdmin ? 'primary' : 'grey-darken-2'"
            label
            size="small"
...
import CreateItemDialog from '@/components/admins/CreateItem.vue'
...
const openDeleteDialog = (item) => {
  useMainStore().handleErr('alert', 'حذف المستخدم', `هل انت متاكد من حذف هذا المستخدم?`, {
    ...item,
    url: 'admins',
  })
  toDelete.value = item
